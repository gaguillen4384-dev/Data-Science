...
\end{itemize}
The dataset exhibited significant \textbf{class imbalance}. Specifically, two classes dominated the distribution: Type 2 Diabetes accounted for $60\%$ of the instances, and Pre-Diabetes accounted for $32\%$.
...
For the Logistic Regression models, a comprehensive regularization comparison was performed using three penalties across separate models: \text{L1} (Lasso), \text{L2} (Ridge), and ElasticNet, to evaluate their impact on generalization and feature parsimony for the multiclass problem.\\
...
\indent The optimal value for the inverse Lasso tuning parameter ($C$) was determined through a 5-fold cross-validation procedure, testing values in the set $\{0.001, 0.01, 1\}$. The best-performing parameter was $C = 0.01$, chosen by optimizing the \textbf{weighted F1-score} as the primary scoring metric.\\
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{knn_multiclass_confusion_matrix.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{knn_roc_curve.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{log_lasso_confusion_matrix.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{log_ridge_roc_curve.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{log_elasticnet_confusion_matrix.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{log_elasticnet_roc_curve.png}}
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|c|c|}
		...
		\hline
		\textbf{Best CV F1 Weighted} & 0.7139 & 0.6995 \\
		\hline
		\textbf{Test F1 Weighted Score} & 0.7146 & 0.7039 \\
		\hline
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|c|c|}
		...
		\hline
		\textbf{Best CV F1 Weighted} & 0.6996 & 0.7475 \\
		\hline
		\textbf{Test F1 Weighted Score} & 0.7039 & 0.7483 \\
		\hline
...
\indent The tables \ref{tab:logreg_l1_l2_performance} and \ref{tab:elasticnet_knn_performance}  show that the \textbf{KNN} model is the best overall performer, achieving the highest scores across all metrics. KNN recorded the superior Test $\mathbf{\text{F}_1}$ \textbf{Weighted Score} ($\mathbf{0.7483}$) and the highest \textbf{Overall Test Accuracy} ($\mathbf{0.7492}$), demonstrating the greatest predictive power and the lowest \textbf{Overall Error Rate} ($\mathbf{0.2508}$) on the unseen test data. The closeness between its Best Cross-Validation ($\text{CV}$) $\text{F}_1$ score ($\mathbf{0.7475}$) and Test $\text{F}_1$ score ($\mathbf{0.7483}$) suggests that the model is well-tuned and generalizing effectively without significant overfitting.\\
\indent The multi-class classification models---including \textbf{KNN}, and \textbf{Logistic Regression} with \textbf{LASSO}, \textbf{Ridge}, and \textbf{Elastic Net} regularization---all exhibit a common and significant challenge: a systematic bias toward the most prevalent class, \textbf{Type 2 Diabetes}, resulting in poor performance on minority classes.\\
\indent This pattern is characterized by over-prediction of Type 2 and a noticeable failure to accurately distinguish less frequent conditions like Gestational and Type 1 Diabetes.\\
\indent Specifically, the LASSO-regularized model suffers the most severe outcome, simplifying the complex five-class problem into a functionally poor binary classifier with zero accuracy for three of the classes due to aggressive feature elimination. The KNN model, while successful with Type 2 and Pre-Diabetes, completely fails to classify Gestational and Type 1 Diabetes, suggesting data scarcity or the unsuitability of the $K$-Nearest Neighbors algorithm for these specific, fine-grained distinctions.

\subsection{Features Importance}

\subsubsection{The Difference between Logistic/$\text{L}_2$ and Logistic/ElasticNet}

\indent The tables \ref{tab:ridge_positive_coefficients}, \ref{tab:elasticnet_positive_coefficients}, \ref{tab:ridge_negative_coefficients} and \ref{tab:ridge_negative_coefficients} of the \textbf{Ridge} and \textbf{ElasticNet} models consistently shows that being female is the strongest positive predictor for \textbf{Gestational diabetes}, while high postprandial glucose is the most significant positive predictor for \textbf{Type 2 diabetes}.\\
...
The \texttt{glucose\_postprandial} feature was universally dominant across all models. While Ridge and ElasticNet provided a more detailed view of its application, both $\text{KNN}$ and $\text{Lasso}$ also utilized it extensively, with $\text{Lasso}$ relying on it as the sole feature for its predictions due to its aggressive feature selection.
...
\indent \textbf{The overall performance for all models was poor, with approximately one out of every four classifications being incorrect}. While regularization techniques like ElasticNet and Ridge yielded superior \textbf{AUC} scores (indicating better discriminatory power), the best overall test accuracy was achieved by $k$-Nearest Neighbors ($\mathbf{0.7492}$), closely followed by LASSO ($\mathbf{0.7398}$), a finding consistent with the weighted $F_1$ metric.\\
