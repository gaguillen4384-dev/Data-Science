...
def pre_tunning_log_ridge(X_train):
    '''
    Sets up the preprocessor and the Ridge-enabled Logistic Regression model.
    Ridge performs L2 regularization.
    '''

    lr = LogisticRegression(
        solver='liblinear',
        penalty='l2', # Use L2 penalty for Ridge regularization
        C=0.1,
        random_state=42,
        max_iter=1000
    )
...
def tunning_log_ridge(pre_tunning_result, X_train, y_train, X_test):
    '''
    Processes x_train and x_test using the defined preprocessor pipeline.
    '''
    # This is identical to your original tunning_log_lasso function
...
def fit_final_log_ridge_model(pre_tunning_result, tunning_results, X_train, y_train):
    '''
    Tunes the Ridge regularization parameter C using RandomizedSearchCV.
    '''
    pipeline = Pipeline([
    ...
    # This range is suitable for L2 (Ridge) as well.
    param_dist = {
...
def run_log_multiclass_classification(X_train,y_train,X_test,y_test):
    ...
    pre_tunning_result = pre_tunning_log_ridge(X_train)
    
    tunning_results = tunning_log_ridge(pre_tunning_result, X_train,y_train,X_test)
    
    random_search = fit_final_log_ridge_model(pre_tunning_result, tunning_results, X_train, y_train)
    end_time = time.time()
    ...
    with open('Results/log_ridge_classification_report.json', 'w') as f:
        json.dump(results, f, indent=4)
        
    class_labels = numpy_object.unique(y_train)
    save_confusion_matrix(y_test, y_pred, class_labels,"Results/log_ridge_confusion_matrix.png")
    save_per_class_roc_curve(y_test, y_prob, class_labels, "Results/log_ridge_roc_curve.png")
