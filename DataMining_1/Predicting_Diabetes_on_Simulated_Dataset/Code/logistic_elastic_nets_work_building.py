...
def pre_tunning_log_elasticnet(X_train):
    '''
    Sets up the preprocessor and the Elastic Net-enabled Logistic Regression model.
    Elastic Net performs both L1 and L2 regularization.
    '''

    lr = LogisticRegression(
        solver='saga', 
        penalty='elasticnet', # Use 'elasticnet' penalty for Elastic Net regularization
        C=0.1,
        l1_ratio=0.5, 
        random_state=42,
        max_iter=5000 
...
def tunning_log_elasticnet(pre_tunning_result, X_train, y_train, X_test):
    '''
    Processes x_train and x_test using the defined preprocessor pipeline.
    '''
...
# --- Final Model Fitting and Tuning ---
def fit_final_log_elasticnet_model(pre_tunning_result, tunning_results, X_train, y_train):
    '''
    Tunes the regularization parameters C and l1_ratio for Elastic Net using RandomizedSearchCV.
    '''
    pipeline = Pipeline([
    ...
    # --- CHANGES FOR ELASTIC NET TUNING ---
    param_dist = {
        # Tune C (Inverse of regularization strength)
        'classifier__estimator__C': numpy_object.logspace(-4, 4, 10), 
        # Tune l1_ratio (Elastic Net mixing parameter: 0=L2, 1=L1)
        'classifier__estimator__l1_ratio': numpy_object.linspace(0.01, 1.0, 10) 
    }
...
def get_cv_metrics(random_search, tunning_results, X_train):
    ...
    best_l1_ratio = random_search.best_params_['classifier__estimator__l1_ratio']
...
def run_log_multiclass_classification(X_train,y_train,X_test,y_test):
    ...
    pre_tunning_result = pre_tunning_log_elasticnet(X_train)
    
    tunning_results = tunning_log_elasticnet(pre_tunning_result, X_train,y_train,X_test)
    
    random_search = fit_final_log_elasticnet_model(pre_tunning_result, tunning_results, X_train, y_train)
    end_time = time.time()
    ...
    results = {
        ...
        'best_l1_ratio': cv_metrics['best_l1_ratio'],
        "total_training_tuning_time_seconds": total_time,
        "elasticnet_selected_features_count": len(cv_metrics['selected_features']),
        "elasticnet_selected_features_list": cv_metrics['selected_features']
    }

    with open('Results/log_elasticnet_classification_report.json', 'w') as f:
        json.dump(results, f, indent=4)
        
    class_labels = numpy_object.unique(y_train)
    save_confusion_matrix(y_test, y_pred, class_labels,"Results/log_elasticnet_confusion_matrix.png")
    save_per_class_roc_curve(y_test, y_prob, class_labels, "Results/log_elasticnet_roc_curve.png")
