...
Safety isn't just a metric; it's a result of the non-linear dynamics between drivers and the road.  

This report investigates the "Severity-Infrastructure Paradox" by shifting the focus from single-variable analysis to "meta-aspects"—the overlapping environmental and temporal conditions that drive accident severity.  

Leveraging a refined Orange County dataset of 140,860 observations, DBSCAN unsupervised learning is used to distinguish 652 dense accident clusters ("Black Spots") from noise.  

The goal of this analysis is to differentiate between infrastructure-led risks and those driven by systemic environmental factors, such as the high-pressure professional commute.

\subsection{\textbf{Dataset Structure}}
\indent Derived from \cite{moosavi2019countrywide}, \cite{moosavi2019accident}, and \cite{fdot_geospatial}, the dataset was procured via streaming APIs from the US Department of Transportation, law enforcement agencies and Florida Department of Transportation. For this analysis, the data was narrowed to 140,860 Orange County-specific observations.\\
...
Within the infrastructure space, analysis of Figure \ref{plot_severity_infrastructure}, reveals that Crossings serve as the primary hotspot for accidents, with a sharp peak at Severity (level of traffic impact) \textbf{Level 2}.

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.30]{plot_severity_infrastructure.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{plot_temporal_hour.pdf}}
...
\indent Throughout these periods, Severity \textbf{Level 2} remains the dominant classification, consistently outnumbering more severe incidents.

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{plot_temporal_day.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.25]{west_cfl_overview.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.25]{k_distance_elbow.pdf}}
...
With $\epsilon$ fixed at 0.35 based on the k-distance analysis, we conducted three trials varying the minPts threshold (40, 50, and 60) to optimize the balance between cluster granularity and noise reduction. The results of these trials are summarized in Table \ref{dbscan_results}.
...
Given that not all accidents are meant to be clustered, 19\% being outliers seems appropriate. This framework provides the spatial foundation required to test whether these 652 identified clusters exhibit severity patterns linked to engineering or the aforementioned meta-variables.
...
The analysis of top clusters (Table \ref{cluster_analysis_refined}) reveals three distinct risk profiles based on the interaction of frequency, severity, and infrastructure presence.\\
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}lccccccl@{}}
		...
		\textit{Freq.} 
		& 137 & 1,133 & 0   & 2 & 5PM & Thu & 454.4 \\
		& 150 & 1,092 & 0   & 2 & 4PM & Fri & 438.0[ \\
		& 228 & 1,151 & 0   & 2 & 4PM & Tue & 461.6 \\
		& 290 & 1,101 & 0   & 2 & 5PM & Tue & 441.6 \\
		& 320 & 1,146 & 0   & 2 & 5PM & Wed & 459.6 \\ \midrule
		\textit{Infra} 
		& 7   & 446   & 892 & 2 & 8AM & Thu & 625.2 \\
		& 46  & 425   & 850 & 2 & 9AM & Tue & 595.8 \\
		& 61  & 402   & 804 & 2 & 7AM & Wed & 563.6 \\
		& 64  & 399   & 798 & 2 & 8PM & Wed & 559.4 \\
		& 159 & 430   & 860 & 2 & 5PM & Mon & 602.8 \\ \midrule
		\textit{Sev.} 
		& 57  & 111   & 0   & 3 & 7AM & Wed & 1.2 \\
		& 62  & 124   & 0   & 3 & 8AM & Wed & 1.2 \\
		& 79  & 144   & 0   & 3 & 8AM & Thu & 1.2 \\
		& 181 & 125   & 0   & 3 & 8AM & Tue & 1.2 \\
		& 263 & 115   & 0   & 3 & 7AM & Tue & 1.2 \\ \bottomrule
	\end{tabular}
	\caption{Top Clusters Grouped by Category and Ordered by ID Ascending}
	\label{cluster_analysis_refined}
\end{table}

\textbf{Score} was calculated using these two formulas. This dual-priority formulas benchmark habitual accident patterns while using those frequencies as a relative marker to highlight systemic risk in high-asset zones.  
\begin{itemize}
	\item \textbf{$P_{freq} = (Frequency \times 0.4) + (Severity_{avg} \times 0.6)$}: Identifies high-volume hotspots to establish a baseline of habitual accident frequency.
	\item \textbf{$P_{infra} = (Infra_{total} \times 0.7) + (Severity_{avg} \times 0.4)$}: Prioritizes complex hubs where high infrastructure density creates a greater risk for diverse transit types (pedestrians, cyclists, and vehicles).
\end{itemize}  
...
\begin{itemize}
	...
	Focuses on high-volume clusters by weighting frequency and total severity, using $P_{freq}$.
	\item \textbf{Impact Profile:} While these clusters maintain high cumulative volumes (Freq $> 1,000$) and moderate severity (\textbf{Level 2}), their priority scores ($\approx 440 \text{--} 460$) serve as a baseline for comparison. 
\end{itemize}


\subsection*{2. Infrastructure-Driven Clusters (Asset-Dense Hubs)}
These clusters are defined by high infrastructure presence (weighted at 70\% of the score).

\begin{itemize}
	\item \textbf{Correlation:} There is a relative ratio between frequency and infrastructure ($Infra \approx 2 \times Freq$), indicating systematic asset allocation.
	...
	\item \textbf{Ranking Criteria:} Under this relative marker, infrastructure-dense hubs emerge as higher priorities, as their built-environment complexity results in significantly higher priority scores ($\approx 560 \text{--} 625$) despite having lower raw accident counts.
\end{itemize}

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{Severity_Cluster_Map.pdf}}
	\caption{Top 5 Severity: I-4 Safety Vacuum}
	\label{Severity_Cluster_Map}
\end{figure} 

\subsection*{3. Severity-Driven Clusters (DBSCAN-Identified Maxima)}
...
\begin{itemize}
	...
	\item \textbf{Infrastructure Absence:} The consistent priority score of 1.2 is a direct result of the Formula $P_{infra}$---($(0 \times 0.7) + (3 \times 0.4)$---highlighting a significant gap in infrastructure at these high-risk spatial coordinates.
\end{itemize}

\begin{table}[htbp]
	\centering
	\label{tab:risk_profiles}
	% Use \resizebox to scale the table to fit the column width exactly
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{@{}llll@{}}
			...
			Structural & Infrastructure & Intermittent & Level 2 (Moderate) \\
			High Risk & Severity & Early Morning & Level 3 (Critical) \\ \bottomrule
		\end{tabular}%
	}
...
The West Orlando corridor offers a granular perspective on the \textit{Severity-Infrastructure Paradox}, acting as a focal point to demonstrate how these macro patterns manifest locally.\\
\vspace{-1.25em}

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{Zoom_Map.pdf}}
	\caption{Top 5 By Frequency: High-volume transit links and weekend activity hubs.}
	\label{Zoom_Map}
\end{figure}  

When comparing Figure \ref{Zoom_Map} and Figure \ref{Zoom_Map_Severity}, a clear spatial shift occurs: while frequency clusters are distributed across various surface-street junctions, the severity hotspots consolidate almost exclusively along the \textbf{I-4} unmanaged pipeline.

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{Zoom_Map_Severity.pdf}}
...
\indent The morning rush transforms these low-infrastructure segments into ``Early Morning High-Risk" zones, where the absence of regulatory assets allows standard transit conflicts to escalate into \textbf{Level 3} outcomes.
...
\newblock Florida GIS Open Data Hub. 
\newblock \textit{FDOT Transportation Data and Analytics Office}. 
\newblock Available at: \url{https://gis-fdot.opendata.arcgis.com/} (Accessed: May 2026).
...
The source code, data, and detailed implementation scripts for this project are available on GitHub (URL: \url{https://github.com/gaguillen4384-dev/Data-Science/tree/main/Spatial/FL_Auto_Accident_Analysis}).
...
\begin{itemize}
	...
	\item \textbf{magick}: Used for image file manipulation on density graph in West Orlando Case Ctudy.
