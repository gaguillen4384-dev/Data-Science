...
\indent While SVD and LFM-SGD are benchmarks, we hypothesize that \textbf{Sparse Coding} is better suited for sparse data. By using an overcomplete dictionary and $L_1$ regularization, it Focuses on the most important data and ignores errors, capturing subtle preferences that traditional global-variance methods may overlook.
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.75]{radar_profile.pdf}}
...
Similarly, \textbf{LFM-SGD} and \textbf{Funk SVD} demonstrate robust ranking capabilities with \textbf{NDCG} values of \textbf{0.80} and \textbf{0.89} respectively, yet yield negligible \textbf{Norm\_Accuracy} scores ($\le 0.07$).\\
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.60]{error_comparison.pdf}}
...
In contrast, \textbf{LFM-SGD} and textbf{Funk SVD} exhibit higher baseline error rates. While their training and test metrics are closely aligned, their higher values indicate less precision in predicting exact ratings compared to Sparse Coding.
...
\textbf{LFM-SGD} proved to be the weakest model. Its \text{NDCG} dropped sharply from $0.86$ on the training set to below $0.80$ on the test set.\\
...
The visualization highlights shown in Figure \ref{graph_precision_vs_cecall} the trade-off between \textbf{Precision (the quality of recommendations)} and \textbf{Recall (the breadth of recommendations)}. \textbf{Sparse Coding} emerges as the superior model, occupying the top-right position with a \textbf{Precision} $\approx 0.90$ and \textbf{Recall} $\approx 0.54$, effectively maximizing relevance while minimizing noise.
...
\textbf{LFM-SGD} occupies the middle tier, yielding a \textbf{Precision} $\approx 0.86$ and \textbf{Recall} $\approx 0.51$. Finally, \textbf{Funk SVD} is the least performant model, sitting at the base of the cluster with a \textbf{Precision} of 0.85 and the lowest \textbf{Recall} at 0.50, capturing the smallest portion of relevant items.
...
\indent Despite the 93.7\% sparsity and high dimensionality, \textbf{Sparse Coding} emerged as the most robust framework, achieving superior precision and generalization.
