...
\title{Characterizing High-Risk Environmental Signatures: An Unsupervised Approach to Predicting Crash Probability}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{temporal_analysis.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{categorical_sensitivity.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.40]{categorical_distribution_weather.pdf}}
...
\textbf{Severity:} Where the severity of most accidents are mostly in Level 2 and 3, with low-impact and high-impact be in minority grouping of the accident's distribution, shown in Figure \ref{severity_detailed}.

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{severity_detailed.pdf}}
	\caption{Severity Level Distribution}
	\label{severity_detailed}
\end{figure} 

\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{Linear_Correlation.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{PCA_Combined_Scree_Plot.pdf}}
...
\end{enumerate}
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{UMAP_Density_Map.pdf}}
...
To identify the distinct environmental signatures associated with accidents probability, the first eight Principal Components ($PC_1$--$PC_8$) are utilized as the input feature space. This ensures that the clustering algorithms operate on the most significant variance in the dataset while mitigating the impact of collinearity among raw environmental variables. Three distinct models are evaluated to identify the "Perfect Storm" clusters:

\begin{itemize}
	\item \textbf{K-Means Clustering \cite{MacQueen1967}:} A centroid-based algorithm that partitions the PC-space into $k$ clusters by minimizing the squared Euclidean distance between data points and their respective cluster centers. This model should provides a clear, geometric partitioning of environmental risk profiles.
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.3]{k-means_diagnostic_plots.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{k-means_pca_PC1_PC2_k.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.50]{gmm_bic_selection_plot.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{gmm_pca_PC1_PC2.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.45]{spectral_eigengap_plot.pdf}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.35]{spectral_pca_PC1_PC2.pdf}}
	\caption{Spectral $PC_1$ vs. $PC_2$}
	\label{spectral_pca_PC1_PC2}
\end{figure} 

This visualization is essential for interpreting how \textbf{Spectral Clustering} aligns with the primary dimensions of variance. Notably, where the \textbf{GMM} (Figure \ref{gmm_pca_PC1_PC2}) identified three overlapping clusters in the central population, the \textbf{Spectral Clustering} model concentrates the majority of its eight clusters within that middle structure.\\
\indent Only two clusters are positioned on the periphery, appearing distinct enough to be isolated from the dense central concentration.
...
\indent The convergence of these metrics provide a case for valid interpretability. The high global scores of \textbf{K-Means} confirm a strong fundamental structure in the data, while the low Davies-Bouldin index for the \textbf{Spectral Clustering} model validates that even at a high granularity ($k=8$), the clusters remain compact and distinct.\\
...
\begin{table}[ht]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{l|c|cc|ccc|c}
			\hline
			\textbf{Cluster} & \textbf{Sev.} & \textbf{Temp} & \textbf{Hum.} & \textbf{Cross.} & \textbf{Junct.} & \textbf{Sign.} & \textbf{Hour} \\ \hline
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{l|c|cc|ccc|c}
			\hline
			\textbf{Cluster} & \textbf{Sev.} & \textbf{Temp.} & \textbf{Vis.} & \textbf{Cross.} & \textbf{Junct.} & \textbf{Sign.} & \textbf{Hour} \\ \hline
